...
StatusType streaming_database::rateRemoveAndAddMovie(Node<Movie>* movieNode, double rating)
{
    ...
    bool vip = movieNode->getValue()->getVipOnly();
    Genre genre = movieNode->getValue()->getGenre();
    int numOfVoters = movieNode->getValue()->getNumOfVoters();
    ...
    currentRating = currentRating * numOfVoters;
...
StatusType streaming_database::viewRemoveAndAddMovie(Node<Movie>* movieNode, int views)
{
    ...
    Genre genre = movieNode->getValue()->getGenre();
    int numOfVoters = movieNode->getValue()->getNumOfVoters();
    int newViews = viewsBeforeAddition + views;
    ...
    Movie* movie = new Movie(movieId, newViews, vip, genre, rating, numOfVoters, false);
    Movie* movieWithFlag = new Movie(movieId, newViews, vip, genre, rating, numOfVoters, true);
...
StatusType streaming_database::user_watch(int userId, int movieId)
{
    ...
    //removing and adding the movie to the trees cause the views on it changed so maybe its "size" changed as well;
...
StatusType streaming_database::group_watch(int groupId,int movieId)
{
    ...
    groupNode->getValue()->addViewToMembers(groupNode->getValue()->getMembers()->getRoot()
                                            ,movieNode->getValue()->getGenre());
